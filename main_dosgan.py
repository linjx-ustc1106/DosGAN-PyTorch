...
def main(config):
    ...
    train_dataset = datasets.ImageFolder(config.train_data_path, train_trans(config))
    
    test_dataset = datasets.ImageFolder(config.test_data_path, test_trans(config))
    data_loader_train = data.DataLoader(dataset=train_dataset, batch_size=config.batch_size, shuffle=True, num_workers=config.num_workers, drop_last=True)
    print('train dataset loaded')
    data_loader_test = data.DataLoader(dataset=test_dataset, batch_size=config.batch_size, shuffle=True, num_workers=config.num_workers, drop_last=True)
    ...
    # Solver for training and testing dosgan.
    solver = Solver(data_loader_train, data_loader_test, config)
...
if __name__ == '__main__':
    ...
    # Directories.
    parser.add_argument('--train_data_path', type=str, default='data/facescrub_train/')
    parser.add_argument('--test_data_path', type=str, default='data/facescrub_test/')
    parser.add_argument('--model_dir', type=str, default='dosgan')
    parser.add_argument('--cls_save_dir', type=str, default='dosgan_cls/models')
